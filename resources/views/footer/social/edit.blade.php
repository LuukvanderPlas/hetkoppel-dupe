...
@section('title', 'Social bewerken')

@section('pageTitle', 'Social bewerken')

@section('content')

    <form action="{{ route('socials.update', ['social' => $social->id]) }}" method="POST"
...
@push('scripts')
    <script>
        ...
        for (let i = 0; i < iconelement.children.length; i++) {
            if (iconelement.children[i].value == iconelement.dataset.old) {
                iconelement.selectedIndex = i;
            }
        }
    </script>
@endpush
